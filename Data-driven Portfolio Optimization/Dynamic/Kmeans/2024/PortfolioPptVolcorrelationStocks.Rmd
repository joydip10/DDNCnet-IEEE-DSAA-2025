---
title: "Optimizing Portfolio Using Data-Driven Risk Measures"
author: "JOY DIP DAS"
date: "28/04/2025"
...
```{r}
nasset=ncol(price_data1)
```

```{r}
# Organize and calculate returns

ret_tidy = price_data1
...
```{r}
plot(train[, 1], type = 'l', col = 'red', main = colnames(train)[1], ylab = '', xlab = '', lwd = 2)
legend("topright", legend = colnames(train)[1], col = "red", lty = 1, lwd = 2)
```
...
```{r}
p1 <- cbind(min_var1[1:5], sqrt(252)*min_var1[6], sqrt(252)*min_var1[7], sqrt(252)*min_var1[12])
p2 <- cbind(min_var2[1:5], sqrt(252)*min_var2[6], sqrt(252)*min_var2[8], sqrt(252)*min_var2[13])
p3 <- cbind(min_var3[1:5], sqrt(252)*min_var3[6], sqrt(252)*min_var3[9], sqrt(252)*min_var3[14])
p4 <- cbind(min_var4[1:5], sqrt(252)*min_var4[6], sqrt(252)*min_var4[10], sqrt(252)*min_var4[15])
p5 <- cbind(min_mad[1:5], sqrt(252)*min_mad[6], sqrt(252)*min_mad[11], sqrt(252)*min_mad[16])
xtable(t(rbind(as.numeric(p1), as.numeric(p2), as.numeric(p3), as.numeric(p4), as.numeric(p5))), digits = 5)
```

```{r}
p1 <- cbind(max_sr1[1:5], sqrt(252)*max_sr1[6], sqrt(252)*max_sr1[7], sqrt(252)*max_sr1[12])
p2 <- cbind(max_sr2[1:5], sqrt(252)*max_sr2[6], sqrt(252)*max_sr2[8], sqrt(252)*max_sr2[13])
p3 <- cbind(max_sr3[1:5], sqrt(252)*max_sr3[6], sqrt(252)*max_sr3[9], sqrt(252)*max_sr3[14])
p4 <- cbind(max_sr4[1:5], sqrt(252)*max_sr4[6], sqrt(252)*max_sr4[10], sqrt(252)*max_sr4[15])
p5 <- cbind(max_sr5[1:5], sqrt(252)*max_sr5[6], sqrt(252)*max_sr5[11], sqrt(252)*max_sr5[16])
xtable(t(rbind(as.numeric(p1), as.numeric(p2), as.numeric(p3), as.numeric(p4), as.numeric(p5))), digits = 5)
```
...
```{r}
asset_columns <- colnames(min_var4)[1:20]

p1 <- min_var4 %>%
  gather(all_of(asset_columns), key = Asset, value = Weights) %>%
  mutate(Asset = as.factor(Asset)) %>%
  ggplot(aes(x = fct_reorder(Asset, Weights), y = Weights, fill = Asset)) +
  geom_bar(stat = 'identity') +
  theme_minimal() +
  labs(x = 'Assets', y = 'Weights', title = "Minimum Risk Portfolio Weights (First 20 Stocks)") +
...
```{r}
asset_columns <- colnames(max_sr4)[1:20]
...
p2 <- max_sr4 %>%
  ...
  geom_bar(stat = 'identity') +
  theme_minimal() +
  labs(x = 'Assets', y = 'Weights', title = "Maximum Sharpe Ratio Portfolio Weights (First 20 Stocks)") +
  scale_y_continuous(labels = scales::percent)
...
```{r}
row.names(Portfolios)
```


```{r}
row.names(Portfolios) <- price_data1$X[-(1:20)]
